...
/**
 * Extension to the Node class that adds support for a reliable, in-order messaging layer.
 * 
 * Nodes that extend this class for the support should use RIOSend and onRIOReceive to send/receive the packets from the
 * RIO layer. The underlying layer can also be used by sending using the regular send() method and overriding the
 * onReceive() method to include a call to super.onReceive()
 */
public abstract class RIONode extends Node {
  private final ReliableInOrderMsgLayer RIOLayer;
   ...
   * 
   * @param from The address from which the message was received
   * @param protocol The protocol identifier of the message
