...
public class Client {
  ...
  public void start() {
    ...
    TwitterProtocol tpRestart =
        new TwitterProtocol(TwitterServer.RESTART, TwitterServer.RESTART, TwitterServer.RESTART, new Entry(tnw.addr)
            .getHash());
    tnw.RIOSend(serverId, Protocol.DATA, tpRestart.toBytes());
  ...
  public void abortCommand() {
    TwitterProtocol tpAbort = new TwitterProtocol(TwitterServer.ABORT, new Entry(tnw.addr).getHash());
    RIOSend(serverId, Protocol.DATA, tpAbort.toBytes());
    isAbortMode = true;
    if (commandQueue.size() == 0) {
