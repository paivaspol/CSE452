...
/**
 * Class that tests the reliable, in-order message layer. Randomly sends messages to other nodes and receives them in
 * order from the RIO layer. This continues until we send at least 100 messages to another node.
 */
public class RIOTester extends RIONode {
  ...
  public void failure(int from, int i) {
    logError("FAILURE OF THE RIO MESSAGE LAYER!!  Received " + i + " instead of " + receivedNums.get(from) + " from "
        + from);
    failed = true;
    fail();
  }
  ...
  /**
   * Method that chooses a random receiver to send the next message, and schedules itself to execute again in the next
   * time step
   */
  public void sendNextNum() {
    ...
    // Send the message
    RIOSend(destAddr, Protocol.RIOTEST_PKT, Utility.stringToByteArray("" + next));
    nextNum.put(destAddr, next + 1);

    // Schedule another send for the next time step
    try {
      Callback cb = new Callback(Callback.getMethod("sendNextNum", this, new String[0]), this, new Object[0]);
      addTimeout(cb, 1);
