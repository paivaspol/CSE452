...
public class FileManager {
	...
	private int getNumCommits() {
		...
		try {
			PersistentStorageReader reader = server
					.getPersistentStorageReader(TwitterServer.REDO_LOGGING_FILENAME);
			String ln = "";
	...
	private void recoverWriteExecution() {
		try {
			...
			if (reader.ready()) {
				...
				PersistentStorageWriter revertFile = server
						.getPersistentStorageWriter(filename, false);
			...
			PersistentStorageWriter tmpFileWriter = server
					.getPersistentStorageWriter(TwitterServer.TEMP_FILENAME,
							false);
	...
	public String handleTransaction(int transactionId, String method,
		...
		retval = handleMethod(transactionId, method, filename, value, retval,
				exec);
		return retval;
	}

	private String handleMethod(int transactionId, String method,
			String filename, String value, String retval,
	...
	private void create(String filename, TransactionalExecution exec)
			throws IOException {
	...
	private void append(String filename, String value,
			TransactionalExecution exec) throws IOException {
		StringBuilder content = new StringBuilder();
	...
	private void deleteLines(String filename, String value,
			TransactionalExecution exec) throws IOException {
	...
	private String read(String filename, String retval,
			TransactionalExecution exec) throws IOException {
		String content = exec.readFile(filename);
		if (content == null) {
			StringBuilder cont = new StringBuilder();
	...
	private String commit(int transactionId, String filename, String retval,
			TransactionalExecution exec) throws IOException {
		int lastTxn = -1;
		if (Utility.fileExists(server.getNode(), filename)) {
			lastTxn = getLastModifiedVersion(filename);
		}
	...
	public void writeToFile(String filename, String content) throws IOException {
		...
		PersistentStorageWriter tempFileWriter = server
